...
def url_for(endpoint, request=None, **values):
...
def parse_project_sections(body_lines, project_id=None, request=None):
    """
...
def index(request):
    ...
    # 传给模板做展示
    return render_template(request, 'index.html', directory_tree=directory_tree)
...
def article(request, filename):
    ...
    with open(full_path, 'r', encoding='utf-8') as f:
        ...
        md = markdown.Markdown(extensions=[
            ...
            # article.html upgrades <pre><code class="language-python"> into a runnable CodeMirror block.
            # Pygments output strips that class and nests spans, so keep fenced-code language classes here.
...
def upload_code(request):
    ...
    code = form.get('code', '')
    # 也可以取一下语言信息
    language = normalize_language(form.get('language', '') or "python")
    # 新增获取模板类型（默认用 editor）
    template_type = form.get('template', 'editor')
    project_payload_raw = form.get('project_payload', '')
    ...
    theme = normalize_theme(form.get('theme', '') or payload_theme)
...
def download_project_zip(request):
    form = request.POST
    project_payload = get_project_payload_for_download(form.get('project_payload', ''))
    filename = sanitize_download_filename(form.get('filename', 'codemark-project.zip'))
    ...
    with zipfile.ZipFile(archive_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        if text_file_count == 0:
            fallback_code = form.get("code", "")
            if fallback_code:
                fallback_language = normalize_language(form.get("language", "") or "python")
                write_zip_bytes(
                    zip_file,
...
@require_GET
def get_shared_asset(request, project_id, asset_path):
    ...
    if not os.path.isfile(abs_asset_path):
        return HttpResponse("File not found", status=404)
    content_type, _ = mimetypes.guess_type(abs_asset_path)
    return FileResponse(open(abs_asset_path, "rb"), content_type=content_type or "application/octet-stream")


@require_GET
def show_shared_code(request, project_id):
    """
    ...
    os.makedirs(sharecode_root, exist_ok=True)
    for folder in os.listdir(sharecode_root):
        ...
        if os.path.isdir(folder_path):
            ...
            if os.path.isfile(possible_path):
                ...
                parsed_project = parse_project_sections(body_lines, project_id=project_id, request=request)
